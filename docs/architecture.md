...
1. 客户端经 HTTP/WebSocket 发送消息；Gateway 经 `BindingRouter` 与 `LaneManager` 选定车道后，调用实现了 `Supervisor` / `ScopedSupervisor` 的团队运行时入口。当前主链路默认由 `team.Registry` 接收请求，再按 `scope/workstream` 复用或创建具体 `team.Manager`。
...
`Gateway`（`internal/gateway/server.go`）将 **HTTP**（健康检查、建会话、Chat、debug、MCP）与 **WebSocket** 统一为对 `Supervisor.HandleRequest` / `ScopedSupervisor.HandleScopedRequest` 的调用；会话由 `SessionManager` 持有 channel/user 元数据，并可携带 `scope/workstream`，便于审计、绑定与长期工作线复用。
...
`SessionManager` 生成的会话 ID 与 `Supervisor` 的 `sessionID` 应对齐传入 `HandleRequest`，以便 Handoff 历史与路由日志按会话聚合。当前 session 还可以显式带 `scope/workstream`，或在后续 continuation 请求中由 `ScopedSupervisor` 解析并绑定到已有工作线。若客户端自建 ID，需保证唯一性与 TLS 下防篡改（生产应配合 auth 中间件）。
...
2. Gateway 从 `SessionManager` 取会话，选择 lane，调用 `team.Registry`（或其他 `ScopedSupervisor` 实现）处理请求，再由 Registry 解析/复用 `scope/workstream` 并转发给具体 `team.Manager`。
